...
def cdi_postprocess(fp_seq, cdi_zip, map_dir=None, plot=False, debug=False):
    ...
    if map_dir:
        ...
        for ix in range(len(cmds_probe_only)):
            ...
            cpx_dm_sim[ix], smp = proper.prop_run('scexao_model', .9, sim_grid,
                                             PASSVALUE={'map':probe_maps[ix],'verbose':debug, 'ix':ix}, QUIET=True)
        # Nulls
        for ix in range(len(cmds_nulls_only)):
            ...
            cpx_null_sim[ix], smp = proper.prop_run('scexao_model', .9, sim_grid,
                                          PASSVALUE={'map': null_maps[ix], 'verbose':debug, 'ix':ix}, QUIET=True)
    ...
    # if debug:
        ...
        #                    vmax=np.max(probe_maps[0])*.8, vmin=np.min(probe_maps[0])*.8
        #                    )
        #     ax.set_title(f"t={(probe_tstamps[ix] - cdi_zip.ts.cmd_tstamps[0]).astype('float')/1e9:.2f}")
        #
        # cax = fig.add_axes([0.91, 0.2, 0.02, 0.6])  # Add axes for colorbar @ position [left,bottom,width,height]
    ...
    #
    #     for ax, ix in zip(subplot.flatten(), range(cdi_zip.ts.n_probes)):
    #         im = ax.imshow(np.abs(cpx_all[ix])**2)
    #         ax.set_title(f"t={(probe_tstamps[ix] - cdi_zip.ts.cmd_tstamps[0]).astype('float')/1e9:.2f}")
    #
    #     cax = fig.add_axes([0.91, 0.2, 0.02, 0.6])  # Add axes for colorbar @ position [left,bottom,width,height]
    #     cb = fig.colorbar(im, orientation='vertical', cax=cax)  #
    ...
    E_pupil = np.zeros((n_nulls*cdi_zip.ts.n_cycles, nx, ny), dtype=complex)
...
if __name__ == '__main__':
    ...
    datf = '/darkdata/kkdavis/mec/Jan2021/Jan2021_test5_1611980038_processed_old.npz'
    run_data = np.load(datf, allow_pickle=True)
    tcube_regcycle = run_data['tcube_regcycle'].tolist()
    fp_seq = tcube_regcycle['cube']
    ...
    grail = cdi_postprocess(fp_seq, cdi_zip,  plot=False, debug=True)
    plt.show()
    dumm=1
