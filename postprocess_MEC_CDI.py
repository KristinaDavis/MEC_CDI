...
def cdi_postprocess(fp_seq, cdi_zip, map_dir=None, plot=False, debug=False):
    ...
    if map_dir:
        ...
        DM_maps = np.zeros(
            (cdi_zip.ts.n_probes, cdi_zip.probe.DM_cmd_cycle.shape[1], cdi_zip.probe.DM_cmd_cycle.shape[2]))
        for nx in range(cdi_zip.ts.n_probes):
            cdi_cmd = datetime_to_unix(cmds_probe_only[nx])
    ...
    for ip in range(n_pairs):
        delta[ip] = (fp_seq[:,:,ip] - fp_seq[:,:,ip + n_pairs]) / 2
...
if __name__ == '__main__':
    ...
    grail = cdi_postprocess(fp_seq, cdi_zip, map_dir, plot=True)
